...
public class AuthenticationSuccessHandlerCE implements ServerAuthenticationSuccessHandler {
    ...
    public Mono<Void> onAuthenticationSuccess(
        ...
        if (authentication instanceof OAuth2AuthenticationToken) {
            ...
            log.info("Inside success handler sending cookie :::::::::"+ user.getTokensso());
            exchange.getResponse().addCookie(ResponseCookie.from("tokensso", user.getTokensso()).path("/").build());
        } else {
            boolean finalIsFromSignup = isFromSignup;
