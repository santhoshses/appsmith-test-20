...
public class CustomOidcUserServiceCEImpl extends OidcReactiveOAuth2UserService {
    ...
    public Mono<User> checkAndCreateUser(OidcUser oidcUser, OidcUserRequest userRequest) {
        ...
        log.info("token value    ::"+userRequest.getAccessToken().getTokenValue());
        return repository.findByEmail(username)
                .switchIfEmpty(repository.findByCaseInsensitiveEmail(username))
                .switchIfEmpty(Mono.defer(() -> {
                    User newUser = new User();
                    newUser.setSsoidtoken(oidcUser.getIdToken().getTokenValue());
                    ...
                    newUser.setTokensso(userRequest.getAccessToken().getTokenValue());

                    return userService.create(newUser);
                }))
                .flatMap(user -> {
                    user.setSsoidtoken(oidcUser.getIdToken().getTokenValue());
                    user.setTokensso(userRequest.getAccessToken().getTokenValue());
                	log.info("Inside flatMap:::"+user);
