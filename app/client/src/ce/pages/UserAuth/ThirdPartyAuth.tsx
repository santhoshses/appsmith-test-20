...
function SocialLoginButton(props: {
  ...
  const openWindow = () => {
    // if (url.includes("oidc")) window.open("oauth2/authorization/oidc");
    if (url.includes("oidc")) window.location.href = url;
  };
  return (
...
function ThirdPartyAuth(props: {
  ...
  const socialLoginButtons = getSocialLoginButtonProps(props.logins).map(
    (item) => {
      ...
      return (
        <div className="hidden" key={item.name}>
          <SocialLoginButton key={item.name} {...item} type={props.type} />;
        </div>
      );
    },
  );
